...
;!function(lairen) {
    ...
    _.ID_FRAGMENT_ROOT = 'lairen_pages';
...
!function($lr/*, undefined*/) {
    ...
    /* 是否支持 Session storage */
    var session_storage_supported           = !! 0;
    var persistent_session_stack_idx_offset = 0;
    ...
    var _STACK_INDEX_       = '_stack_idx_',
        _EL_                = '_el_',
        _LAYOUT_            = '_layout_',
        _LAYOUT_ID_         = '_layout_id_',
        _STATE_             = '_state_', /* initial value is INITIALIZING */
        /* 可能的父级 page */
        _PARENT_            = '_parent_';
    
    var _RENDER_CALLED_     = '_render_called_';
    ...
    function _page_sequence_token(id, args) {
        return 'lai-page__' + hash_code( _calculate_derive_key( id, args ) );
    }
    ...
    function _prepare() {
        ...
        _FRAGMENT_TEMPLATE = $( '<div class="lai-page"></div>' );
    ...
    /* NOTE(XCL): This just reference the android page state. */
    var INITIALIZING        = 0; /* Not yet created. */
    var CREATED             = 1; /* Created. */
    var FRAGMENT_CREATED    = 2; /* The page has finished its creation. */
        ...
        if ( scroll_dom.length ) {
            ...
            var options = {
                ...
                /* FIXME(XCL): 解决 form field 在获取 focus 后，点击任意元素都会唤出键盘的问题
                 preventDefault: lairen.os.ios || false,*/
                keyBindings: true
            };
     ...
     * @private
     */
    function _has_page_content/* _has_inner_view */(page) {
        if ( is_sandbox_mode() ) {
            var layout_id = get_layout_id.call( page );
            
            //console.log( 'layout_id: ' + layout_id + ', _dom: ' + _global_dom_container[ layout_id ] );
    ...
    function get_layout_id() {
        return String( this[ _EL_ ][ _LAYOUT_ID_ ] );
    }
    ...
    function _render_with_html(data) {
        ...
        if ( is_sandbox_mode() ) {
            ...
            if ( _current == this ) {
                ...
                if ( data[ 'reload' ] )
                    $lr._viewport.children('.page-ui').remove();
                
                $lr._viewport.append( data[ _HTML ] );
            } else {
    ...
    function _request_go(id, args, from_uri, /* fromUser, zIndex, */animation) {
        ...
        if ( _is_support_multi_instance( id ) )
            _go_next_with_multi_mode( id, args, /*fromUser*/0, from_uri, animation );
        else
            _go_next( id, args, /*fromUser*/0, from_uri, animation );
    }
    ...
    function _build_transition(fx, forward) {
        ...
        return {
            rear:   rear,
            front:  front,
            ease:   scheme['ease'] || $lr.cubic_bezier
        };
    }
     ...
     * @param {boolean} reverse 标识是否执行反向的切换效果, 如果下一个即将呈现的 page 支持 animation
     * @private
     */
    function _perform_go(from_uri, animation, reverse) {
        ...
        /* onVisibilityChanged */
        /* 标识是否 Page 的内容已加载完成 */
        var has_page_content_in_next = _has_page_content( next );
        ...
        if ( is_sandbox_mode() ) {
            ...
            if ( current ) {
                ...
                _hide( current /*, _FROM_STACK_YES, endTransNeeded */,
                    transit,
         ...
         * 有可能不一致.
         */
        if ( is_sandbox_mode() || ! postCommitTrans ) {
            /* 标识 trans 完成 */
            _end_trans();
    ...
    function _setup_current_state(target, from_uri) {
        ...
        if ( from_uri )
            ...
            _push_history_state( state, title, hash );
    ...
    function _perform_back(animation) {
        ...
        if ( ! _is_derive( _current ) ) {
            ...
            //console.log( "_is_same_args: past => %s,\n\r\t\t\t present => %s",
            //    JSON.stringify( currentlyArgs ),
            //    JSON.stringify( _current[ _ROUTE_ARGS ] )
            //);
    ...
    function _schedule_destroy(who) {
        ...
        if ( ! is_sandbox_mode() ) {
            trigger = /*$.fx.speeds.slow * 10 + 25*/$.fx.speeds.slow * ($.fx.off ? .2 : 1.2);
        }
     ...
     *
     * @param target
     * @param fromStack 是否从 stack 中取得 page
    ...
    /**
     * FIXME: 相同 hash 不同参数不列为一个新的 Back stack record, 另外一旦浏览器
     * reload 这里便会重置。
     */
    ...
    function _push_history_state(state, title, url_or_hash) {
        //console.log( "before _push_state: arguments => %o, \n\t\t\t\t_back_stack => %o",
        //    JSON.stringify( arguments ),
        //    JSON.stringify( _back_stack )
        //);
        ...
        //console.log( "after _push_state: _back_stack => %o\n\r",
        //    JSON.stringify( _back_stack )
        //);
    }
            ...
            Object.keys( _sandbox_dom_container ).forEach( function (key) {
                ...
                if ( z ) {
                    ...
                    if ( collection_of_page.length )
                        result_collection = result_collection.concat( collection_of_page );
                }
            } );
    ...
    function _settle_pending_page_if_necessary(id) {
        ...
        var pending         = _pending_page_info;
        
        _pending_page_info  = void 0;
        
        _request_go( pending.id, pending.args, pending.fromUri, pending.animation );
    }
    ...
    function _register(id, /* properties */props) {
        ...
        /* 标识是否需要填充默认 page, 如果没有首个 page 被呈现 */
        var havePagePresented = _has_page();
        ...
        /* 解析后的 hash, lairen.ui.home -> lairen/ui/home */
        frag[ _ROUTE ] = _make_id_as_xpath_route( id );
        ...
        _register_triggers_if_necessary( frag, props );
    ...
    function _is_same_args(l, r) {
        return $lr.stringify( l ) === $lr.stringify( r );
    }
    ...
    var _pop_state_handler = function(event) {
        /*console.log( "before backward: state => %o, \n\t\t\t\t_back_stack => %o",
        ...
        /* FIXME(XCL): 处理 Browser reload 之后发生的 back 行为 */
        if ( ! _runtime_back_stack.length ) {
            if ( event.state ) {
                location.reload();
                return;
        ...
        //console.log( "after backward: _back_stack => %s\n\r", JSON.stringify( _back_stack ) );
    };
    ...
    function collect_current_page_elements() {
        return $lr._viewport.children( '.page-ui' );
    }
    ...
    function restore_page_elements() {
        ...
        if ( saved_elements ) {
            if ( $lr.isString( saved_elements ) )
                saved_elements = $( saved_elements );
            
            saved_elements.appendTo( $lr._viewport );
            _sandbox_dom_container[ layout_id ] = void 0;
        }
    }
     ...
     *
     * $Page.define(['home', {title:'Home'}, 'about', {title:'About'}]);
     *
     * @public
     */
    $lr.page = function(id, props) {
        /* TODO: alias, short */
        if ( $lr.isArray( id ) ) {
            var pages = [];
            
            for ( var idx in id ) {
                pages.push( _register( id[ idx ], {} ) );
            }
    ...
    function _is_page_navigation_link($link) {
        return $link.is( '.lai-page__nav' ) || $link.is( '.lai-page__back' );
    }
    ...
    $( function() {
        ...
        var boot_loader = function () {
            ...
            if ( ! _has_page_content( first ) ) {
                create_view( first );
            } else {
                pre_render.call( first );
                rendered.call( first );
                
                /* 标识 content 已经载入 */
                _mark_content_was_loaded( first );
            }
            ...
            if ( history_api_supported ) {
                ...
                /* TODO(XCL): 当 sessionStorage 不被支持时需要启用这项操作 */
                var state = _new_state();
                //state[ _BSR_IDX ] = persistent_session_stack_idx_offset;
                //
                //history.replaceState(
                //    state,
                //    '',
        ...
        function prepare_checking_for_scroll_end() {
            ...
            scroll_end_checker = setTimeout( function () {
                if ( last_scroll_y === window.scrollY ) {
                    ...
                    $lr._viewport.trigger( 'scroll:end', last_scroll_y );
                }
            }, 100 );
        }
